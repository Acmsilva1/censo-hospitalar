...
export class CsvCensoRepository {
    dataDir;
    masterFilePath;
    ...
    normalizeTipoName(type) {
        ...
        if (t.includes('BLACK') || t.includes('BLK'))
            return 'APT BLK';
    ...
    async getPatients() {
        const map = new Map();
        if (!fs.existsSync(this.ocupacaoFilePath))
            return map;
        ...
        if (unitIdIdx === -1 || patientsIdx === -1 || tipoIdx === -1)
            return map;
        for (let i = 1; i < lines.length; i++) {
            ...
            if (this.UNIT_ID_REMAP[unitId])
                unitId = this.UNIT_ID_REMAP[unitId];
    ...
    async getAvailability() {
        const map = new Map();
        if (!fs.existsSync(this.availabilityFilePath))
            return map;
    ...
    async getComplement() {
        const map = new Map();
        if (!fs.existsSync(this.complementFilePath))
            return map;
...
//# sourceMappingURL=CsvCensoRepository.js.map