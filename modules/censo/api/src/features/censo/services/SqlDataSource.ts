...
class DuckDbDataSource implements SqlDataSource {
  ...
  async initialize(): Promise<void> {
    ...
    for (const table of tables) {
      ...
      let sourceSql = '';
      if (fs.existsSync(pq)) sourceSql = `read_parquet('${quoteSqlPath(pq)}')`;
      else if (fs.existsSync(csv)) sourceSql = `read_csv_auto('${quoteSqlPath(csv)}')`;
...
class PostgresDataSource implements SqlDataSource {
  private pool: Pool;
  constructor() {
  ...
  async query<T extends Row = Row>(sql: string, params: unknown[] = []): Promise<T[]> {
    ...
    return (res.rows || []).map((r: unknown) => normalizeRow(r as Row)) as T[];
  }
}

let instance: SqlDataSource | null = null;
export function getDataSource(): SqlDataSource {
  if (!instance) {
