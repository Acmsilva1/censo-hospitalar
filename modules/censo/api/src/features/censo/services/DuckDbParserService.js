...
export class DuckDbParserService {
    ...
    async parseDataset() {
        ...
        const rows = await this.dataSource.query(`SELECT * FROM censo_consolidado`);
        const tree = {};
        rows.forEach(row => {
            const hospitalName = UNIT_ID_TO_NAME[row.unitId] || row.hospitalName;
            // Blacklist Filter
            if (HOSPITAL_BLACKLIST.some(t => hospitalName.toUpperCase().includes(t) ||
            ...
            if (row.ieSituacao === 'I' && !row.patientId)
                return;
            // Status e Emojis
...
//# sourceMappingURL=DuckDbParserService.js.map