...
function normalizeBedName(name) {
    if (!name)
        return 'N/A';
    ...
    if (parts.length >= 2) {
        ...
        if (/^\d+$/.test(prev) && /^[A-Z0-9]+$/.test(last)) {
            if (last.startsWith(prev) && last.length > prev.length)
                cleaned = last;
...
function normalizeTypeName(type) {
    ...
    if (t.includes('UTI') || t.includes('UPC') || t.includes('PACIENTE CRITICO') || t.includes('PACIENTE CRÍTICO'))
        return 'UTI';
    if (t.includes('APT') || t.includes('APARTAMENTO'))
        return 'APT';
    if (t.includes('ENF') || t.includes('ENFERMARIA'))
        return 'ENF';
...
export class CsvParserService {
    ...
    constructor(repository) {
        ...
        this.masterFilePath = repository.getMasterFilePath?.()
    ...
    async parseDataset() {
        ...
        return new Promise((resolve, reject) => {
            const tree = {};
            const processedBedKeys = new Set();
            const sectorToHospitalMap = new Map();
            ...
            if (!this.masterFilePath.toLowerCase().endsWith('.csv')) {
                return reject(new Error('[Parser] Master em Parquet — use DuckDbParserService (fluxo DuckDB). CsvParserService só lê CSV.'));
            }
            fs.createReadStream(this.masterFilePath)
                ...
                if (UNIT_ID_REMAP[unitId])
                    unitId = UNIT_ID_REMAP[unitId];
                ...
                if (HOSPITAL_BLACKLIST.some(t => hospitalName.toUpperCase().includes(t) || sectorRaw.includes(t) || unitId === t))
                    return;
                ...
                if (processedBedKeys.has(dedupKey)) {
                    ...
                    const existingBeds = tree[targetHospital]?.[floorOrArea]?.[areaType];
                    if (existingBeds) {
                ...
                let patientAge = null;
                let patientSex = null;
                let patientEmoji = null;
                let rawDtAlta = availability?.dtAltaMedico ?? null;
                let rawDtEntrada = availability?.dtEntrada ?? null;
                ...
                if (patientId) {
                    ...
                    if (comp) {
                        if (!patientName)
                            patientName = comp.name;
                        patientAge = comp.age;
                        patientSex = comp.sex;
                        if (!rawDtAlta)
                            rawDtAlta = comp.dtAltaMedico ?? null;
                        if (!rawDtEntrada)
                            rawDtEntrada = comp.dtEntrada ?? null;
                ...
                if (!tree[targetHospital])
                    tree[targetHospital] = {};
                if (!tree[targetHospital][floorOrArea])
                    tree[targetHospital][floorOrArea] = {};
                if (!tree[targetHospital][floorOrArea][areaType])
                    tree[targetHospital][floorOrArea][areaType] = [];
                const bed = {
                    id: normalizedBed,
                    status,
                    patientId,
                ...
                tree[targetHospital][floorOrArea][areaType].push(bed);
                ...
                patientMap.forEach((p, key) => {
                    ...
                    if (uId === '1' && !processedBedKeys.has(key)) {
                        ...
                        const areaType = 'UTI';
                        if (!tree[hospitalName])
                            tree[hospitalName] = {};
                        if (!tree[hospitalName][floorOrArea])
                            tree[hospitalName][floorOrArea] = {};
                        if (!tree[hospitalName][floorOrArea][areaType])
                            tree[hospitalName][floorOrArea][areaType] = [];
                ...
                for (const hosp in tree) {
                    for (const floor in tree[hosp]) {
                        for (const type in tree[hosp][floor]) {
                            tree[hosp][floor][type].sort((a, b) => a.id.localeCompare(b.id, undefined, { numeric: true, sensitivity: 'base' }));
                        }
                    }
                }
                ...
                for (const hosp in tree) {
                    ...
                    for (const floor in tree[hosp]) {
                        ...
                        let floorOccupied = 0;
                        for (const type in tree[hosp][floor]) {
                            const beds = tree[hosp][floor][type];
                            beds.forEach(b => {
                                floorTotal++;
                                globalTotal++;
